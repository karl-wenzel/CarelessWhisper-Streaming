...
class AlignedTextGridDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ".wrd" in tg_path:
            ...
            tg_path = Path(tg_path) if not os.path.exists(Path(tg_path).with_suffix('.punc')) else Path(tg_path).with_suffix('.punc')
            tg = textgrid.openTextgrid(tg_path, includeEmptyIntervals=False)
            text_intervals = tg.getTier("words") if tg_path.suffix == '.TextGrid' else tg.getTier("words_punctuated")
